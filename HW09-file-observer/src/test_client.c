...
static const char* socket_path = "sock.socket";
...
int main() {
    ...
    strcpy(send_msg, "SUBSCRIBE");
    printf("msg len: %lu", strlen(send_msg));
    if( send(sock, send_msg, strlen(send_msg)*sizeof(unsigned char), 0 ) == -1 ) {
        printf("Client: Error on send() call \n");
    }
